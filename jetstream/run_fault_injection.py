"""Fault-injection experiment, orchestrated from the local machine.

Timeline (all from when Spark app starts):
    t=0       launch producers + Spark on driver01 (8 workers)
    t=KILL    docker rm -f spark-worker on exec01 (kill it hard)
    t=RESTORE re-launch spark-worker on exec01
    t=END     stop everything, collect logs

Outputs three measurements for the report:
    - time from kill to next successful batch (recovery latency)
    - time from restore to re-registration (rejoin latency)
    - whether any data was lost (compared via gap_count + total_records)
"""
import os
import sys
import time
...
def main():
    ...
    print(f"\n==> sleeping {wait_until_restore}s while Spark recovers without the worker")
    ...
    print(f"\n==> sleeping {remain}s for the restored worker to re-register")
    time.sleep(remain)
    ...
    print("==> compare with epoch in the master log to compute recovery / rejoin times")
