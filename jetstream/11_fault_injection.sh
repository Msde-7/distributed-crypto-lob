...
#   - start producers + Spark with all 8 workers
#   - 30s of clean operation to establish baseline
#   - SSH to one executor, kill its spark-worker container
#   - watch master + spark-app logs to measure time-to-resume
#   - restart the worker, watch re-registration
#
# Reports two numbers for the report:
#   - time from kill to next successful Spark batch completion
#   - time from worker restart to re-registration with master
#
# REQUIRES: ssh from driver01 to exec01 with the same key
# Easiest path: this script runs the kill via docker on the LOCAL host (driver01)
#   for the spark-master, and uses python on driver01 to remotely kill on exec01.
...
TOTAL_DURATION=180
KILL_AT=60          # seconds in, kill the worker
RESTORE_AT=120      # seconds in, restart it
...
echo
echo "######################################################"
echo "### FAULT INJECTION at t=${KILL_AT}s (epoch ${KILL_TS})"
echo "######################################################"
ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 \
    exouser@${TARGET_EXEC_IP} 'docker rm -f spark-worker' 2>&1 | tail -2
echo "==> killed spark-worker on ${TARGET_EXEC_IP}"

echo "==> watching for next successful batch (Spark should re-task to surviving workers)"
...
echo
echo "######################################################"
echo "### RESTORING worker at t=${RESTORE_AT}s (epoch ${RESTORE_TS})"
echo "######################################################"
ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 \
...
echo "==> sleeping ${REMAIN}s for the restored worker to re-register and resume"
sleep ${REMAIN}
...
echo "restore timestamp: ${RESTORE_TS}"
echo
echo "==> last 5 master events around the kill window"
docker logs spark-master 2>&1 | grep -E "Removing worker|Registering worker" | tail -10
echo
echo "==> Spark task losses + recoveries"
grep -E "Lost task|Lost executor|removed|reschedul" logs/fault/spark-app.log | head -20
echo
echo "==> Spark batch IDs and timing around the kill"
